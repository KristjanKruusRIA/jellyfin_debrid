        ...
        if hasattr(self, "media") and hasattr(self.media, "tmdbId"):
            try:
                ...
                if movie_details:
                    ...
                    if (
                        hasattr(movie_details, "releaseDate")
                        and movie_details.releaseDate
                    ):
                    ...
                    if (
                        hasattr(movie_details, "externalIds")
                        and hasattr(movie_details.externalIds, "imdbId")
                        and movie_details.externalIds.imdbId
                    ):
        ...
        if hasattr(self, "media") and hasattr(self.media, "tmdbId"):
            try:
                ...
                if tv_details:
                    ...
                    # Add IMDB ID from externalIds if available
                    if (
                        hasattr(tv_details, "externalIds")
                        and hasattr(tv_details.externalIds, "imdbId")
                        and tv_details.externalIds.imdbId
                    ):
            ...
            try:
                ...
                for element in response.results:
                    ...
                    try:
                        ...
                        user = (
                            element.requestedBy.displayName
                            if hasattr(element, "requestedBy")
                            and hasattr(element.requestedBy, "displayName")
                            else None
                        )
                ...
                for element in response.results:
                    if not any(
                        x.id == element.id and x.updatedAt == element.updatedAt
        ...
        if len(users) > 0 and len(api_key) > 0:
            ...
            try:
                ...
                for element_ in response.results:
                    if (
                        ...
                        if element not in self.data:
                            ...
                            if not any(
                                x.id == element_.id
                                and x.updatedAt == element_.updatedAt
                                for x in last_requests
                            ):
                                last_requests.append(element_)
                ...
                for element in last_requests[:]:
                    if element.id not in (x.id for x in response.results):
                ...
                for existing_item in self.data[:]:
                    ...
                    api_result = next(
                        (
                            ...
                            if hasattr(existing_item, "request_id")
                            and x.media.id == existing_item.request_id
                        ),
                        None,
                    )
                    if api_result:
                        ...
                        status_allowed = (
                            [str(api_result.media.status)] in allowed_movie_status
                            if api_result.type == "movie"
                            else [str(api_result.media.status)] in allowed_show_status
                        )
