    ...
    def get_trakt_watched_from_api(
        ...
        try:
            response = requests.get(
                "https://api.trakt.tv/sync/watched/movies", headers=headers
            )
    ...
    def sync_watched_movies(self, watched_movies: List[Dict]):
        ...
        for idx, item in enumerate(watched_movies, 1):
            ...
            if not tmdb_id:
                ...
                if imdb_id:
                    print(
                        f"[{idx}/{len(watched_movies)}] No TMDB ID for '{title}', searching by IMDB ID..."
                    )
                    tmdb_id = self.search_tmdb_by_imdb(imdb_id)

                if not tmdb_id:
                    print(
                        f"[{idx}/{len(watched_movies)}] ✗ Skipping '{title}': No TMDB ID found"
                    )
                    skip_count += 1
                    continue
            ...
            if tmdb_id in existing_blacklist:
                print(
                    f"[{idx}/{len(watched_movies)}] ⊘ Skipping '{title}': Already blacklisted"
                )
                skip_count += 1
                continue
...
def main():
    ...
    # Trakt API settings
    parser.add_argument(
        "--trakt-client-id", help="Trakt API client ID (required for API mode)"
    )
    parser.add_argument(
        "--trakt-client-secret", help="Trakt API client secret (required for API mode)"
    )
    parser.add_argument(
        "--trakt-access-token", help="Trakt API access token (required for API mode)"
    )

    # JSON file settings
    parser.add_argument(
        "--json-file", help="Path to Trakt JSON export file (required for JSON mode)"
    )
