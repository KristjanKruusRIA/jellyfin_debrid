...
class BlacklistRemover:
    ...
    def remove_from_blacklist(self, tmdb_id: int, title: str, year: Optional[str]) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                print(f"✗ Failed to remove {title} {year_str}: {response.status_code} - {response.text}")
                return False
        except Exception as e:
    ...
    def process_removals(
        ...
        movie_blacklist = [item for item in blacklist if item.get('mediaType') == 'movie']
        ...
        print(f"Processing removals for year range: {year_gte or 'any'} - {year_lte or 'any'}")
        print(f"Dry run mode: {self.dry_run}\n")
        
        for idx, item in enumerate(movie_blacklist, 1):
            blacklist_id = item.get('id')  # This is the blacklist database ID
            tmdb_id = item.get('tmdbId')   # This is the TMDB movie ID
            title = item.get('title', 'Unknown')
            
            # Progress indicator every 100 items
            if idx % 100 == 0:
                print(f"Progress: {idx}/{len(movie_blacklist)} items checked, {removed_count} removed, {skipped_count} skipped...")
...
def main():
    ...
    parser.add_argument(
        '--year',
        type=int,
        help='Specific year to remove (e.g., 2026)'
    )
    parser.add_argument(
        '--year-gte',
        type=int,
        help='Minimum release year to remove (e.g., 2026)'
    )
    parser.add_argument(
        '--year-lte',
        type=int,
        help='Maximum release year to remove (e.g., 2027)'
    )
    ...
    if not args.year and not args.year_gte and not args.year_lte:
        parser.error("At least one of --year, --year-gte, or --year-lte must be provided")
