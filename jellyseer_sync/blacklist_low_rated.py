...
class LowRatedBlacklist:
    def __init__(
        ...
        self.blacklist_countries = (
            [c.upper() for c in blacklist_countries] if blacklist_countries else []
        )
    ...
    def get_discover_movies(
        ...
        if vote_count_lte:
            vote_count_filter += (
                ...
                else f"vote_count<={vote_count_lte}"
            )
        vote_count_filter = vote_count_filter or "any"
        print(
            f"Filters: genre={genre}, {year_filter}, vote_avg>={vote_avg_gte}, {vote_count_filter}"
        )
        ...
        while page <= max_pages:
            try:
                ...
                print(
                    f"  Page {page}: Found {len(results)} movies (total: {len(all_movies)})"
                )
    ...
    def add_to_blacklist(
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                print(
                    f"⊘ Already blacklisted: {title} (TMDB: {tmdb_id}, IMDb: {imdb_score})"
                )
                return True
            else:
                print(
                    f"✗ Failed to blacklist {title}: {response.status_code} - {response.text}"
                )
                return False
        except Exception as e:
    ...
    def process_movies(self, movies: List[Dict], existing_blacklist: set):
                ...
                if movie_details:
                    ...
                    if self.min_runtime:
                        ...
                        if runtime > 0 and runtime < self.min_runtime:
                            print(
                                f"[{idx}/{len(movies)}] {title}: Runtime {runtime}min < {self.min_runtime}min → BLACKLISTING"
                            )
                    ...
                    if self.blacklist_countries:
                        production_countries = movie_details.get(
                            "productionCountries", []
                        )
                        country_codes = [
                            pc.get("iso_3166_1", "").upper()
                            for pc in production_countries
                        ]
                        blacklisted_country = next(
                            (c for c in country_codes if c in self.blacklist_countries),
                            None,
                        )
                        if blacklisted_country:
                            country_name = next(
                                (
                                    pc.get("name", blacklisted_country)
                                    for pc in production_countries
                                    if pc.get("iso_3166_1", "").upper()
                                    == blacklisted_country
                                ),
                                blacklisted_country,
                            )
                            print(
                                f"[{idx}/{len(movies)}] {title}: Country '{country_name}' is blacklisted → BLACKLISTING"
                            )
                            if self.add_to_blacklist(tmdb_id, title, None):
        ...
        if self.min_runtime:
            print(
                f"  Blacklisted (runtime < {self.min_runtime}min): {runtime_blacklist_count}"
            )
        if self.blacklist_countries:
...
def main():
    ...
    # Discover filters
    parser.add_argument("--genre", type=int, help="Genre ID (e.g., 27 for horror)")
    parser.add_argument(
        "--year-gte", type=int, help="Minimum release year (e.g., 2020)"
    )
    parser.add_argument(
        "--year-lte", type=int, help="Maximum release year (e.g., 2024)"
    )
    parser.add_argument(
        "--vote-avg-gte", type=float, help="Minimum TMDB vote average (e.g., 5.0)"
    )
    parser.add_argument(
        "--vote-count-gte", type=int, help="Minimum TMDB vote count (e.g., 51)"
    )
    parser.add_argument(
        "--vote-count-lte", type=int, help="Maximum TMDB vote count (e.g., 1000)"
    )
    parser.add_argument(
        "--min-runtime",
        type=int,
    ...
    parser.add_argument(
        "--max-pages", type=int, default=50, help="Maximum pages to fetch (default: 50)"
    )
    ...
    if args.skip_blacklist_check:
        ...
        print(
            f"Skipped {len(movies) - len(movies_to_check)} already blacklisted movies\n"
        )
