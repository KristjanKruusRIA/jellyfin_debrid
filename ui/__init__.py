...
# Explicit standard-library imports to satisfy linters
import json
import os
import sys
import time
...
from ui.ui_print import set_log_dir, ui_cls, ui_print
    ...
    while not back:
        ...
        if len(content.classes.ignore.ignored) == 0:
            ...
            if len(library_services) == 0:
                print(
                    "No library collection service configured. Please configure a library service first."
                )
                time.sleep(3)
                return
...
def setup():
    if os.path.exists(config_dir + "/settings.json"):
        if os.path.getsize(config_dir + "/settings.json") > 0 and os.path.isfile(
            ...
            if settings["Show Menu on Startup"] == "false" or service_mode:
                return False
            load()
            return True
...
def save(doprint=True):
    ...
    try:
        ...
        print()
        print(
            "Error: It looks like jellyfin_debrid can not write your settings into a config file. Make sure you are running the script with write or administator privilege."
        )
        print()
...
def load(doprint=False, updated=False):
    ...
    if "Library ignore services" not in settings:
...
def update_available():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/itsToggle/jellyfin_debrid/main/ui/ui_settings.py",
            timeout=0.25,
        )
...
def update(settings, version):
    ...
    print(
        "There has been an update to jellyfin_debrid, which is not compatible with your current settings:"
    )
    print()
    print(version[1])
    print()
...
def threaded(stop):
    ...
    if len(library_services) == 0:
        ui_print(
            "No library collection service configured. Exiting.", ui_settings.debug
        )
        return
    library = library_services[0]()
    ...
    if len(library) > 0:
        ...
        if _debrid.downloading:
            ...
            for download_id in _debrid.downloading:
                ...
                base_query = (
                    download_id.split(" [")[0] if " [" in download_id else download_id
                )
                # Check if any library item matches this query
                for lib_item in library:
                    if (
                        hasattr(lib_item, "query")
                        and lib_item.query().lower() == base_query.lower()
                    ):
                        items_to_remove.append(download_id)
                        ui_print(
                            f"[ui] removing from downloading list (already in library): {download_id}"
                        )
                        break
        ...
        ui_print(
            f"[ui] debug: initial scan check - debrid.downloading length: {len(_debrid.downloading)}, ignore_queue length: {len(content.classes.media.ignore_queue)}",
            ui_settings.debug,
        )
        if _debrid.downloading:
            ui_print(
                f"[ui] debug: debrid.downloading contents: {_debrid.downloading}",
                ui_settings.debug,
            )
        if content.classes.media.ignore_queue:
            ui_print(
                f"[ui] debug: ignore_queue contents: {[str(x) for x in content.classes.media.ignore_queue]}",
                ui_settings.debug,
            )
                ...
                if hasattr(element, "download"):
                    try:
                        ...
                        if element in content.classes.media.ignore_queue:
                            ...
                            try:
                                cooldown_min = int(
                                    os.getenv("DOWNLOAD_COOLDOWN_MINUTES", "30")
                                )
                            except Exception:
                                cooldown_min = 30
                            if element.is_in_cooldown():
                                ...
                                ui_print(
                                    f"[ui] skipping download (in cooldown {rem}s): {element_name}",
                                    ui_settings.debug,
                                )
                            else:
                                content.classes.media.ignore_queue += [element]
                                ui_print(
                                    f"[ui] debug: added to ignore_queue: {element_name}, queue length: {len(content.classes.media.ignore_queue)}",
                                    ui_settings.debug,
                                )
                                try:
                                    ...
                                    match = next(
                                        (
                                            x
                                            for x in content.classes.media.ignore_queue
                                            if element == x
                                        ),
                                        None,
                                    )
                                    if match:
                                        ...
                                        ui_print(
                                            f"[ui] debug: removed from ignore_queue: {element_name}, queue length: {len(content.classes.media.ignore_queue)}",
                                            ui_settings.debug,
                                        )
                                    else:
                                        ui_print(
                                            f"[ui] warning: element not found in ignore_queue for removal: {element_name}, queue: {[str(x) for x in content.classes.media.ignore_queue]}"
                                        )
                                    try:
                                        ...
                                        pass
                        ui_print(
                            f"[ui] finished downloading: {element_name}",
                            ui_settings.debug,
                        )
                    except Exception as e:
                        import traceback
                        ...
                        has_seasons = (
                            hasattr(element, "Seasons")
                            if element_type == "show"
                            else "N/A"
                        )
                        tb = traceback.format_exc()
                        ui_print(
                            f"[ui] error downloading {element_name} (type={element_type}, has_Seasons={has_seasons}): {str(e)}",
                            ui_settings.debug,
                        )
    ...
    while not stop():
        try:
            ...
            if timeout_counter >= regular_check:
                ...
                ui_print(
                    f"[ui] debug: scheduled scan check - debrid.downloading length: {len(_debrid.downloading)}, ignore_queue length: {len(content.classes.media.ignore_queue)}",
                    ui_settings.debug,
                )
                if _debrid.downloading:
                    ui_print(
                ...
                if _debrid.downloading or content.classes.media.ignore_queue:
                    ui_print(
                        "[ui] downloads in progress, skipping scheduled scan",
                        ui_settings.debug,
                    )
                    t0 = time.time()
                else:
                            ...
                            if jellyseerr_requests.update():
                                ...
                                for element in new_watchlists:
                                    if hasattr(element, "download"):
                                        ...
                                        if (
                                            element
                                            in content.classes.media.ignore_queue
                                        ):
                                            ui_print(
                                                f"[ui] skipping download (already in progress): {element_name}",
                                                ui_settings.debug,
                                        ...
                                        try:
                                            ...
                                            if match:
                                                content.classes.media.ignore_queue.remove(
                                                    match
                                                )
                                ui_print("done")
                            t0 = time.time()
