...
def download_from_existing_torrents(query_title, is_show=False):
    ...
    """
    try:
        ui_print(f"[rd_downloader] Checking existing RD torrents for: {query_title}", debug="true")
        ...
        for torrent in torrents:
            ...
            if query_clean in torrent_name or regex.search(query_clean, torrent_name):
                ui_print(f"[rd_downloader] Found matching torrent: {torrent.filename}", debug="true")
                
                # Check if torrent is ready (downloaded)
                if torrent.status in ['downloaded', 'seeding', 'uploading']:
                    # Get torrent info with links
                    torrent_info = rd.get(f'https://api.real-debrid.com/rest/1.0/torrents/info/{torrent.id}')
                        ...
                        for file in torrent_info.files:
                            if hasattr(file, 'selected') and file.selected == 1:
                                files_to_download.append({
                                    'name': file.path.split('/')[-1] if hasattr(file, 'path') else torrent.filename,
                                    'size': file.bytes if hasattr(file, 'bytes') else 0,
                                    'id': file.id,
                                    'url': None
                                })
                    ...
                    if not files_to_download:
                        files_to_download.append({
                            ...
                            'size': torrent.bytes if hasattr(torrent, 'bytes') else 0,
                            'id': 0,
                            'url': None
                        })
                    ...
                    if not best_file:
                        ui_print("[rd_downloader] No suitable file found in torrent", debug="true")
                        continue
                    ...
                    for link in torrent_info.links:
                        try:
                            response = rd.post('https://api.real-debrid.com/rest/1.0/unrestrict/link',
                                             {'link': link})
                    ...
                    if not unrestricted_links:
                        ui_print("[rd_downloader] Could not get download links", debug="true")
                        continue
                    
                    # Download the best file
                    download_url = unrestricted_links[0] if len(unrestricted_links) == 1 else unrestricted_links[best_file.get('id', 0)] if best_file.get('id', 0) < len(unrestricted_links) else unrestricted_links[0]
                    
                    result = downloader.download_file(download_url, best_file['name'], is_show)
                    
                    if result:
                        ui_print(f"[rd_downloader] Successfully downloaded from existing RD torrent", debug="true")
                        return True
                    
                elif torrent.status in ['queued', 'downloading', 'magnet_conversion']:
                    ui_print(f"[rd_downloader] Torrent is still downloading on RD: {torrent.status}", debug="true")
                    return False
        
        ui_print(f"[rd_downloader] No matching existing torrents found", debug="true")
        return False
        
    except Exception as e:
        ui_print(f"[rd_downloader] Error checking existing torrents: {str(e)}", debug="true")
        return False
