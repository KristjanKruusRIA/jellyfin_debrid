    ...
    try:
        ...
        if os.path.exists(temp_file):
            try:
                ...
                if file_age > 3600:  # More than 1 hour old - abandoned download
                    os.remove(temp_file)
                    ui_print("[downloader] Removed old temp file", debug="true")
                ...
                for chunk in response.iter_content(
                    ...
                    if chunk:
                        ...
                        if downloaded - last_reported >= 500 * 1024 * 1024:
                            ...
                            ui_print(
                                f"[downloader] Progress: {downloaded_mb:.0f}MB / {total_mb:.0f}MB ({percent:.1f}%)",
                                debug="true",
                            )
        ...
        try:
            if expected_size and expected_size > 0:
                ...
                threshold = max(
                    int(expected_size * 0.05), 10 * 1024 * 1024
                )  # 5% or 10MB
                if diff > threshold:
                    ui_print(
                        f"[downloader] Error: downloaded size ({downloaded} bytes) differs from expected ({expected_size} bytes) by more than threshold ({threshold} bytes). Discarding file.",
                        debug="true",
                    )
                    # Clean up temp file
                    if os.path.exists(temp_file):
        ...
        ui_print(
            f"[downloader] Download complete: {file_size_mb:.2f}MB ({file_size_bytes} bytes) - {filename}",
            debug="true",
        )
        return dest_path
...
def parse_filename(filename):
    ...
    if is_show:
        ...
        if title_match:
            info["title"] = (
                title_match.group(1).replace(".", " ").replace("_", " ").strip()
            )
    else:
        ...
        if title_match:
            ...
            title_match = regex.search(
                r"^(.+?)[\.\s]+(720p|1080p|2160p|4K)", filename, regex.I
            )
            if title_match:
                info["title"] = (
                    title_match.group(1).replace(".", " ").replace("_", " ").strip()
...
def is_video_file(filename):
    ...
    if regex.search(
        r"(2160p|4K|1080p|720p|480p|BluRay|WEB-?DL|HDTV|WEBRip)", filename, regex.I
    ):
...
def select_best_file(files_list):
    ...
    if not filtered_files:
        ...
        ui_print(
            f"[downloader] Warning: No video files found in release. Available files: {available_files}",
            debug="true",
        )
        return None
    ...
    for file in filtered_files:
        ...
        scored_files.append(
            {"file": file, "quality_score": quality_score, "size": size, "info": info}
        )
    ...
    best = scored_files[0]
    ui_print(
        f"[downloader] Selected best file: {best['file'].get('name')} "
...
def organize_path(filename, is_show=False, element=None):
    ...
    if info["is_show"] or is_show:
        ...
        if element and hasattr(element, "parentIndex"):
            ...
            element
            and hasattr(element, "index")
            and hasattr(element, "type")
            and element.type == "season"
        ):
...
def download_from_realdebrid(release, element):
    ...
    try:
                            ...
                            files_to_download.append(
                                {
                                    "name": file.name,
                                    "size": file.size
                                    * 1000000000,  # Convert GB back to bytes
                                    "id": file.id,
        ...
        if is_show:
            ...
            if not filtered_files:
                ui_print(
                    "[downloader] No suitable video files found to download",
                    debug="true",
                )
                return False
            ...
            if not best_file:
                ui_print(
                    "[downloader] No suitable file found to download", debug="true"
                )
                return False
        ...
        ui_print(
            f"[downloader] Error in download_from_realdebrid: {str(e)}", debug="true"
        )
        return False
